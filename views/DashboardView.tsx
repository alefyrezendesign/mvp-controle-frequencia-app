...
import { Unit, FrequencyCategory, AttendanceStatus } from '../types';
...
const DashboardView: React.FC<DashboardViewProps> = ({ store, selectedUnit }) => {
  ...
  const unitMembers = useMemo(() => 
    store.members.filter((m: any) => m.unitId === selectedUnit.id && m.active),
  ...
  const stats = useMemo(() => {
    ...
    const memberStats = unitMembers.map((m: any) => {
  ...
  const monthJustifications = useMemo(() => {
    return store.attendance
      ...
      .map((r: any) => ({
        ...r,
        memberName: store.members.find((m: any) => m.id === r.memberId)?.name || 'Membro desconhecido'
      }))
...
const SummaryCard = ({ label, value, icon, clickable, onClick, subText }: any) => (
